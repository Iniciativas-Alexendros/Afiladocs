...
interface TemplateDownloadReadyProps {
  userName: string
  productName: string
  downloadUrl: string
  expiresAt: string
...
export function TemplateDownloadReady({
  userName,
  productName,
  downloadUrl,
  expiresAt,
  portalUrl,
}: TemplateDownloadReadyProps) {
  return (
    <Html lang="es">
      <Head />
      <Preview>Tu {productName} está listo para descargar</Preview>
      <Body style={main}>
        <Container style={container}>
          <Text style={heading}>afiladocs</Text>
          <Text style={title}>Tu documento está listo</Text>
          <Hr style={hr} />
          <Text style={body}>Hola {userName},</Text>
          <Text style={body}>
            Aquí tienes tu <strong>{productName}</strong>. El enlace es de un solo uso seguro y caduca el{' '}
            <strong>{new Date(expiresAt).toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' })}</strong>.
          </Text>
          ...
          <Text style={body}>
            Si el enlace expira antes de que puedas descargarlo, regénéralo desde tu portal:
          </Text>
...
const detailsBox = { backgroundColor: '#f3f4f6', borderRadius: '6px', padding: '12px 16px', margin: '16px 0', wordBreak: 'break-all' as const }
